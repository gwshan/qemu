...
static void virt_cpu_set_properties(Object *cpuobj, Error **errp)
{
    ...
    /* set topology only for `pre-plugged` vCPUs */
    if (!vms->acpi_dev) {
        object_property_set_int(cpuobj, "socket-id",
                                virt_get_socket_id(CPU(cpuobj)->cpu_index),
                                NULL);
        object_property_set_int(cpuobj, "cluster-id",
                                virt_get_cluster_id(CPU(cpuobj)->cpu_index),
                                NULL);
        object_property_set_int(cpuobj, "core-id",
                                virt_get_core_id(CPU(cpuobj)->cpu_index),
                                NULL);
        object_property_set_int(cpuobj, "thread-id",
                                virt_get_thread_id(CPU(cpuobj)->cpu_index),
                                NULL);
    }

    cpu_slot = virt_get_possible_cpu_arch_id(CPU(cpuobj)->cpu_index);
    ...
    if (vms->psci_conduit != QEMU_PSCI_CONDUIT_DISABLED) {
        ...
        /* start secondary CPUs in a PSCI powered-down state */
